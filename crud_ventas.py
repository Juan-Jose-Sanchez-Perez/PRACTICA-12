...
def main(page: ft.Page):
    ...
    def nueva_venta_tab(e=None):
        ...
        def registrar_venta(e):
            ...
            try:
                cursor.execute("INSERT INTO Ventas (telefono_cliente, id_empleado, total, fecha_venta) VALUES (%s, %s, %s, %s)", (tel_to_use, empleado_dropdown.value, float(total_text.value.split('$')[1]), datetime.now()))
                ...
                for codigo_barras, dropdown_articulo, cantidad, precio, _ in venta_items:
                    ...
                    if art_id and cantidad.value:
                        ...
                        cursor.execute("INSERT INTO Detalles_Venta (id_venta, id_articulo, cantidad, subtotal) VALUES (%s, %s, %s, %s)", (id_venta, art_id, qty, qty * float(precio.value)))
                conn.commit(); output.value = "Venta registrada correctamente"
